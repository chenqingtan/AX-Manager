...
import com.googlecode.d2j.util.zip.*;
...
public class MultiDexFileReader implements BaseDexFileReader
 {
  ...
  public static BaseDexFileReader open ( byte[] file ) throws IOException
   {
	return new DexFileReader ( file );   
   }
		...
		for ( ZipEntry e : zipFile.entries ( ) )
		 {
		  ...
		  if ( entryName.startsWith ( "classes" ) && entryName.endsWith ( ".dex" ) )
		   {
			if ( !dexFileReaders.containsKey ( entryName ) )
			 { // only the first one
